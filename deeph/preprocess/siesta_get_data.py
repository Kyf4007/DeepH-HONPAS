...
import h5py
import json
from scipy.io import FortranFile, savemat
...
def siesta_parse(input_path, output_path):
    ...
    # .ORB_INDX file can have connected col_0 and col_1 when index > 100000, split them
    # DeepH+HONPAS
    # .ORB_INDX file can have connected col_0 and col_1 when index > 100000, split them
    orb_indx=[]
    with open('{}/{}.ORB_INDX'.format(input_path, system_name), 'r') as fid_orb:
        lines = fid_orb.readlines()[3:-19]
    for i in lines:
        data = i.split()
        #data = [0 if isinstance(item, str) else item for item in data]
        if len(data)==15:
            indx=str(data[0])
            indx1=int(indx[:6])
            indx2=int(indx[-6:])
            data=[indx1] + [indx2] + data[1:]
        data[3]=0
        data[9]=0
        data[10]=0
        data=[float(j) for j in data]
        orb_indx.append(data)
    orb_indx=np.array(orb_indx)
    #savemat('/public1/home/keyf/DeepH/DeepH_HONPAS/test.mat',{'orb':orb_indx})
    #raise CustomException("run")
