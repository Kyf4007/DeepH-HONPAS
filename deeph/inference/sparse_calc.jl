...
function format_array(arr::Vector{Float64}; group_size=10)
    lines = [join(arr[i:min(i+group_size-1, end)], " ") for i in 1:group_size:length(arr)]
    return join(lines, "\n")  
end
...
function main()
    ...
    if calc_job == "band"
        ...
        close(f)
        # DeepH + HONPAS
        # output band in siesta/HONPAS format
        println("now output siesta/HONPAS format")
        f = open(joinpath(parsed_args["output_dir"], "honpas.bands"),"w")
        Printf.@printf(f, "%.15e\n", fermi_level)
        dim = size(kpaths,1)
        klength = 0
        for i in 1:dim
            kstart = kpaths[i][1] * rlat[:,1] + kpaths[i][2] * rlat[:,2] + kpaths[i][3] * rlat[:,3] 
            kend = kpaths[i][4] * rlat[:,1] + kpaths[i][5] * rlat[:,2] + kpaths[i][6] * rlat[:,3]
            klength += norm(kend-kstart)
        end
        Printf.@printf(f,"%.15f  %.15f\n",0.0,klength * Bohr2Ang)
        Printf.@printf(f,"%.15f  %.15f\n",minimum(minimum(egvals)),maximum(maximum(egvals)))
        Printf.@printf(f,"%d  %d  %d\n",size(egvals,1),1,size(egvals,2)) # "1": single spin
        #calculate the culmulative k point moving distance
        k_list_total = nothing
        for i = 1:size(kpaths, 1)
            pnkpts = num_ks[i]
            kstart = kpaths[i][1:3]
            kend = kpaths[i][4:6]
            ...
            if k_list_total == nothing
                k_list_total = k_list
            else
                k_list_total = [k_list_total; k_list]
            end
        end
        cumulative_dist = 0.0
        k_cumulative = zeros(Float64,size(k_list_total, 1),1);
        k_cumulative[1] = 0.0
        global prev_kvec
        for i = 1:size(k_list_total, 1)
            kvec = k_list_total[i,:]
            if i > 1
                cumulative_dist += norm(kvec - prev_kvec)
                k_cumulative[i] = cumulative_dist
            end
            prev_kvec = kvec
        end
        # print the band energies
        idx_k = 1
        for i = 1:size(kpaths, 1)
            pnkpts = num_ks[i]
            kstart = kpaths[i][1:3]
            ...
            for j = 1:pnkpts
                kvec = k_list[j,:]
                println(f, k_cumulative[idx_k], " ", format_array(egvals[:, idx_k]))
                idx_k += 1
            end
        end
        println(f, "      ", size(kpaths, 1)+1)
        println(f, "      ", "0.0")
        pnkpts_total = 0
        for i = 1:size(kpaths, 1)
            pnkpts_total += num_ks[i]
            println(f, "      ", k_cumulative[pnkpts_total])
        end
        close(f)
        matwrite(joinpath(parsed_args["output_dir"], "band.mat"), Dict("k_data" => k_data,"num_ks" => num_ks, "kpaths" => kpaths, "rlat" => rlat, "egvals" => egvals))
        println("save band.mat")
        # DeepH + HONPAS
    elseif calc_job == "dos"
        fermi_level = config["fermi_level"]
        max_iter = config["max_iter"]
