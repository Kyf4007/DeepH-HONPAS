...
function main()
    ...
    if calc_job == "band"
        ...
        close(f)
        # DeepH + HONPAS
        # output band in siesta format
        println("now output siesta format")
        f = open(joinpath(parsed_args["output_dir"], "siesta.bands"),"w")
        Printf.@printf(f, "%.15e\n", fermi_level)
        dim = size(kpaths,1)
        klength = 0
        for i in 1:dim
        ...
        for i = 1:size(kpaths, 1)
            ...
            for j = 1:pnkpts
                kvec = k_list[j,:]
                println(f, std_out_array(egvals[:, idx_k]))
                idx_k += 1
            end
        end
        close(f)
        matwrite(joinpath(parsed_args["output_dir"], "result.mat"), Dict("k_data" => k_data,"num_ks" => num_ks, "kpaths" => kpaths, "rlat" => rlat, "egvals" => egvals))
        println("save result.mat")
        # DeepH + HONPAS
    elseif calc_job == "dos"
        fermi_level = config["fermi_level"]
        max_iter = config["max_iter"]
